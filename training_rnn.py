...
def main(args, dt_object):
    ...
    for log_name in logs:
        ...
        while (sum(p.numel() for p in model.parameters() if p.requires_grad)) < args.max_parameters:
            ...
            model = models.SequentialDecoder(hidden_size=args.hidden_dim + number_of_additions,
                                             num_layers=args.n_layers,
                                             dropout_prob=args.dropout_prob,
                                             vocab_size=attributes_meta[0]['vocabulary_size'],
                                             attributes_meta=attributes_meta,
        ...
        model = models.SequentialDecoder(hidden_size=args.hidden_dim + number_of_additions - 1,
                                         num_layers=args.n_layers,
                                         dropout_prob=args.dropout_prob,
                                         vocab_size=attributes_meta[0]['vocabulary_size'],
                                         attributes_meta=attributes_meta,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_criterion_threshold', help='save_criterion_threshold', default=1000.0, type=float)
