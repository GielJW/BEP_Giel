...
def generate_suffixes_ae(checkpoint_file, log_file, args, path, iteration):
    with open('results/ae/Sepsis%20Cases%20-%20Event%20Log.xes/experiment_parameters{}.json'.format(iteration), 'r') as f:
    ...
    with open(os.path.join(path, 'evaluation_parameters{}.json'.format(iteration)), 'a') as fp:
        json.dump(vars(args), fp)
        fp.write('\n')

    model = models.SequentialAutoEncoder(hidden_size=hidden_dim,
                                         num_layers=n_layers,
...
def generate_suffixes_rnn(checkpoint_file, log_file, args, path, iteration):

    with open('results/rnn/helpdesk.csv/experiment_parameters{}.json'.format(iteration), 'r') as f:
        parameters = json.load(f)
        training_batch_size = parameters['training_batch_size']
        validation_batch_size = parameters['validation_batch_size']
        n_layers = parameters['n_layers']
    ...
    model = models.SequentialDecoder(hidden_size=hidden_dim,
                                     num_layers=n_layers,
                                     dropout_prob=dropout_prob,
                                     vocab_size=attributes_meta[0]['vocabulary_size'],
                                     attributes_meta=attributes_meta,
...
def generate(datetime, model_type, args, iteration):
    ...
    for log_directory in sorted(os.scandir(path), key=lambda x: x.name):
        ...
        if 'helpdesk' in log_path:
            continue
        if 'sepsis_rnn' in log_path:
            continue
        if 'sepsis_ae' in log_path:
            continue
        if 'helpdesk_rnn' in log_path:
            continue
        if 'helpdesk_ae' in log_path:
            continue
        if 'Best' in log_path:
            continue
        ...
        print(log_path)
        checkpoint_path = os.path.join(log_path, 'checkpoints'+str(iteration))
        ...
        matching_checkpoint_files = [s for s in checkpoint_files]
        try:
            checkpoint_file = matching_checkpoint_files[0]
        except:
            break
        split_log_file_path = os.path.join(log_path, 'split_log_{}.json'.format(iteration))

        with open(split_log_file_path) as f_in:
            log_file = json.load(f_in)

        if model_type == 'ae':
            predictions = generate_suffixes_ae(os.path.join(checkpoint_path, checkpoint_file), log_file, args, log_path, iteration)
            ...
            predictions = generate_suffixes_rnn(os.path.join(checkpoint_path, checkpoint_file), log_file, args, log_path, iteration)
        ...
        with open(os.path.join(log_path, 'suffix_generation_result_' + str(iteration) + '.json'), 'w') as fp:
            json.dump(predictions, fp)

        del predictions


def evaluate_generation(datetime, model_type, iteration):
    path = os.path.join('results', model_type)
    ...
    for log_directory in sorted(os.scandir(path), key=lambda x: x.name):
        ...
        with open(os.path.join(log_path, 'suffix_generation_result_' + str(iteration) + '.json')) as f_in:
        ...
        with open(os.path.join(log_path, 'suffix_evaluation_result_dls_mae_' + str(iteration) + '.json'), 'w') as fp:
    ...
    for log_directory in os.scandir(path):
        ...
        with open(os.path.join(log_path, 'suffix_evaluation_result_dls_mae_' + str(iteration) + '.json')) as f_in:
            suffix_evaluation_result = json.load(f_in)

        suffix_evaluation_results[model_type][log_directory.name] = suffix_evaluation_result

    with open(os.path.join(path, 'suffix_evaluation_result_dls_mae_' + str(iteration) + '.json'), 'w') as fp:
...
def main(args, dt_object):
    ...
    #         generate(datetime=datetime, model_type=model_type, args=args, iteration=i)
    # if args.dls_evaluation:
    #     for i in range(0, 999):
    #         try:
    #             evaluate_generation(datetime, model_type, iteration=i)
    #         except:
    #             pass
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hidden_dim', help='hidden state dimensions', default=192, type=int)
    ...
    parser.add_argument('--training_batch_size', help='number of training samples in mini-batch', default=2031, type=int)
    parser.add_argument('--validation_batch_size', help='number of validation samples in mini-batch', default=2031, type=int)
    ...
    parser.add_argument('--dropout_prob', help='dropout_prob', default=0.8039682984373921, type=float)
