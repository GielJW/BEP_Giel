...
def main(args, dt_object):
    ...
    for log_name in logs:
        ...
        for experiment_it in range(0, 1000):
            if not os.path.exists(os.path.join(path, 'experiment_parameters{}.json'.format(experiment_it))):
                experiment_version = experiment_it
                break
        ...
        with open(os.path.join(path, 'split_log_{}.json'.format(experiment_version)), 'w') as f:
        ...
        # with open(os.path.join(path, 'experiment_parameters{}.json'.format(experiment_version)), 'a') as fp:
        ...
        # number of increases to layer size
        number_of_additions = 0
        print('no. of triainable parameters: {}'.format(sum(p.numel() for p in model.parameters() if p.requires_grad)))
        while (sum(p.numel() for p in model.parameters() if p.requires_grad)) < args.max_parameters:
            number_of_additions += 1
            model = models.SequentialAutoEncoder(hidden_size=args.hidden_dim + number_of_additions,
                                             num_layers=args.n_layers,
                                             dropout_prob=args.dropout_prob,
                                             vocab_size=attributes_meta[0]['vocabulary_size'],
                                             attributes_meta=attributes_meta,
                                             ...
                                             pad_token=args.pad_token,
                                             nb_special_tokens=attributes_meta[0]['nb_special_tokens'])
            print('no. of triainable parameters: {}'.format(
                sum(p.numel() for p in model.parameters() if p.requires_grad)))

        model = models.SequentialAutoEncoder(hidden_size=args.hidden_dim + number_of_additions - 1,
                                         num_layers=args.n_layers,
                                         dropout_prob=args.dropout_prob,
                                         vocab_size=attributes_meta[0]['vocabulary_size'],
                                         attributes_meta=attributes_meta,
        ...
        vars(args)['hidden_dim'] = number_of_additions
        with open(os.path.join(path, 'experiment_parameters{}.json'.format(experiment_version)), 'a') as fp:
            json.dump(vars(args), fp)
            fp.write('\n')


        training_log_filename = "training_figures_{}.csv".format(experiment_version)
        ...
        os.makedirs(os.path.join(path, 'checkpoints{}'.format(experiment_version)))
            ...
            if args.early_stopping:
                ...
                if len(total_validation_losses) > early_stopping_var:
                    if np.all(np.array(total_validation_losses)[-(early_stopping_var - 1):] >
                        ...
                        checkpoint_name = 'model-' + "{:.4f}".format(
                            model_to_save['total_validation_loss']) + '_epoch-' + str(
                            model_to_save['epoch']) + '_date-' + dt_object.strftime("%Y%m%d%H%M") + '.pt'
                        ...
                        }, os.path.join(path, 'checkpoints{}'.format(experiment_version), checkpoint_name))
